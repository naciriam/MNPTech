...
def main():
    print("Demarrage de l'indexation de la Base de Connaissances...")
    ...
    docs = loader.load()
    print(f"{len(docs)} fichiers charges avec succes.")

    # 2. Découpage du texte en morceaux (chunks)
    print("Decoupage des documents en cours...")
    text_splitter = MarkdownTextSplitter(chunk_size=1000, chunk_overlap=200)
    splits = text_splitter.split_documents(docs)
    print(f"{len(splits)} chunks generes.")
    ...
    print("Chargement du modele de vectorisation local (FastEmbed)...")
